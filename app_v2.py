"""
Calculadora de Value at Risk (VaR) — v3.3 (Professional Edition)
Trabalho Final | Modelagem Aplicada ao Mercado Financeiro
Melhorias: Isolamento estrito de blocos CSS para prevenção de SyntaxError em f-strings.
"""
...
# Injeção de CSS seguro sem f-string para evitar quebra de aspas triplas
st.markdown("""
<style>
...
.stTextInput label, .stNumberInput label, .stDateInput label, .stSelectbox label {
    color: #64748b !important; font-size: 0.75rem !important; font-weight: 600 !important; letter-spacing: 0.05em !important;
}
...
/* Custom Tab Styling */
.stTabs [data-baseweb="tab-list"] { gap: 8px; border-bottom: 1px solid #1e293b !important; padding-bottom: 4px; }
...
.stTabs [aria-selected="true"] { background: #0f172a !important; color: #22d3ee !important; }

/* Componentes de Cards Profissionais */
.kpi-card {
    background: #0f172a; border: 1px solid #1e293b; border-radius: 10px; padding: 1.5rem; height: 100%;
}
.kpi-label { color: #64748b; font-size: 0.75rem; text-transform: uppercase; font-weight: 600; letter-spacing: 0.05em; }
.kpi-value { font-size: 1.75rem; font-weight: 700; font-family: 'JetBrains Mono', monospace; margin: 0.25rem 0; }
.var-card {
    background: #0f172a; border: 1px solid #1e293b; border-left: 4px solid var(--accent); border-radius: 10px; padding: 1.5rem;
}
.section-title {
    color: #f8fafc; font-size: 1.25rem; font-weight: 700; margin: 2rem 0 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid #1e293b;
}
</style>
...
# ===================== SIDEBAR =====================
st.sidebar.markdown('<h3 style="color:#f8fafc; font-weight:700; margin-bottom: 1rem">⚡ RISK LAB</h3>', unsafe_allow_html=True)
...
if calcular:
    with st.spinner("Buscando dados de mercado via Yahoo Finance..."):
        ...
        try:
            ...
            if len(qtds) < len(convert_tickers): 
                qtds += [0] * (len(convert_tickers) - len(qtds))
        except:
            qtds = [1000] * len(convert_tickers)
        
        quantidades = dict(zip(convert_tickers, qtds))
        precos_var = precos_full.loc[pd.to_datetime(data_ini):]
        retornos = precos_var.pct_change().dropna()
        ultimos = precos_var.iloc[-1]
        
        v_acoes = sum(quantidades[t] * ultimos[t] for t in convert_tickers)
        pesos = np.array([(quantidades[t] * ultimos[t]) / v_acoes for t in convert_tickers])
        ret_port = retornos[convert_tickers].dot(pesos)
        ...
        opt_ativo_clean = opt_ativo.strip()
        S0 = ultimos[opt_ativo_clean] if opt_ativo_clean in ultimos.index else ultimos.iloc[0]
        sig_an = retornos[opt_ativo_clean].std() * np.sqrt(252) if opt_ativo_clean in retornos.columns else 0.25
        ...
        st.session_state["tickers"] = convert_tickers
        st.session_state["pesos"] = pesos
        st.session_state["opt_ativo"] = opt_ativo_clean
...
if st.session_state.get("analise_pronta", False):
    ...
    with tabs[1]:
        ...
        ax.axvline(limite_var, color=PRIMARY, linestyle='-', linewidth=1.5, label="VaR Limiar")
    ...
    with tabs[4]:
        ...
        versoes = [
            {"Versão": "v3.3", "Data": "Maio 2026", "Melhoria": "Desacoplamento estrito de strings CSS de escopo interpretado pelo Python."},
            {"Versão": "v3.2", "Data": "Maio 2026", "Melhoria": "Ajuste fino de sintaxe léxica do compilador e normalização de arrays de ticks."},
    ...
    <div style="text-align:center; padding: 8rem 2rem 5rem 2rem;">
        <span style="font-size:3rem; padding:1.2rem 1.5rem; background:#0f172a; border: 1px solid #1e293b; border-radius:50%">📉</span>
        <h2 style="margin-top:2rem; font-weight:700; letter-spacing:-0.03em;">Risk Lab — Gestão de Risco Quantitativo</h2>
        <p style="color:#64748b; font-size:0.95rem; max-width:520px; margin: 0.75rem auto 1.5rem; line-height:1.5">Defina a composição do portfólio e horizontes de probabilidade na barra lateral para inicializar os motores de cálculo.</p>
    </div>
    """, unsafe_allow_html=True)
